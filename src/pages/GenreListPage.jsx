...
export default function GenreListPage() {
  ...
  const getDiscoverMovies = async (id, page) => {
    try {
      ...
      setMovie(prevMovies => [...prevMovies, ...data.results]); // Adiciona os novos filmes ao array existente
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const handleClickCard = (id) => {
    ...
    setPage(1);
    setId(id);
    setMovie([]); // Limpa a lista de filmes ao selecionar um novo gênero
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="sm:grid flex flex-col items-center lg:grid-cols-5 sm:grid-cols-3 grid-cols-1 lg:px-5 my-5 gap-y-10 lg:gap-x-5 gap-x-16 w-full max-w-screen-xl">
        {showMovie ? (
          genre.map(nome => (
            <GenreCard key={nome.id} nome={nome} onclick={() => handleClickCard(nome.id)} />
          ))
        ) : (
          <>
            {movie.map(nome => (
              <MovieCard key={nome.id} {...nome} props={'items-start'} />
            ))}
            <div className="flex justify-center w-full col-span-full mt-5">
